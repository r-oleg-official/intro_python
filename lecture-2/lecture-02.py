...
# разделителей не будет. Данные будут добавлены слитно.
data.writelines(colors)
# после окончания работы нужно закрыть файл. Во избежание утечек памяти.
data.close()
...
print(new_string('!', 5))   # !!!!!
print(new_string('!'))      # !!!
# 12. Python распознает тип данных в момент вызова ф-ции,
print(new_string(4))
...
def concatenatio(*params):  # для этого перед аргументом "*".
    ...
    # Если передать числа, логика поломается. Здесь исп. склеивание строк.
    return res
...
# Error. Т.к. определили число (int) и не получится достучаться до 0-го эл-та.
a = (3)
print(a[0])
...
# {'up': 'uprow', 'left': 'leftrow', 'down': 'downrow', 'right': 'rightrow'}
print(dictionary)
...
# В Python можно создать отд сущность, которая будет организовывать хранение
...
colors.add('red')
# {'red', 'green', 'blue'}. Ошибок не будет и добавления не будет.
print(colors)
colors.add('gray')
...
# ok. Удаление эл-та, если эл-та нет, то ошибки не будет.
colors.discard('red')
print(colors)       # {'green', 'blue', 'gray'}
...
a = {1, 2, 3, 5, 8}
b = {2, 5, 8, 13, 21}
# c = {1, 2, 3, 5, 8}. Создать новое множ-во на основе имеющегося.
c = a.copy()
